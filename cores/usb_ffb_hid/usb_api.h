#ifndef USBffb_HID_h_
#define USBffb_HID_h_
...
extern uint8_t wheel_report_data[12];

class usb_wheel_class
{
        public:
        usb_wheel_class() { manual_mode = 0; }
        inline void button(uint8_t button, bool val) {
                button--;
                uint8_t mask = (1 << (button & 7));
                if (val) {
                        if (button < 8) wheel_report_data[0] |= mask;
                        else if (button < 16) wheel_report_data[1] |= mask;
                        else if (button < 24) wheel_report_data[2] |= mask;
                        else if (button < 32) wheel_report_data[3] |= mask;
                } else {
                        mask = ~mask;
                        if (button < 8) wheel_report_data[0] &= mask;
                        else if (button < 16) wheel_report_data[1] &= mask;
        ...
        inline void X(uint16_t val) {
                ...
                wheel_report_data[4] = (wheel_report_data[4] & 0x0F) | (val << 4);
                wheel_report_data[5] = (wheel_report_data[5] & 0xC0) | (val >> 4);
                if (!manual_mode) send_now();
        }
        inline void Y(uint16_t val) {
                if (val > 1023) val = 1023;
                wheel_report_data[5] = (wheel_report_data[5] & 0x3F) | (val << 6);
                wheel_report_data[6] = (val >> 2);
                if (!manual_mode) send_now();
        }
        inline void position(uint16_t x, uint16_t y) {
                if (x > 1023) x = 1023;
                if (y > 1023) y = 1023;
                wheel_report_data[4] = (wheel_report_data[4] & 0x0F) | (x << 4);
                wheel_report_data[5] = (x >> 4) | (y << 6);
                wheel_report_data[6] = (y >> 2);
                if (!manual_mode) send_now();
        }
        inline void Z(uint16_t val) {
                if (val > 1023) val = 1023;
                wheel_report_data[7] = val;
                wheel_report_data[8] = (wheel_report_data[8] & 0xFC) | (val >> 8);
                if (!manual_mode) send_now();
        }
        inline void Zrotate(uint16_t val) {
                if (val > 1023) val = 1023;
                wheel_report_data[8] = (wheel_report_data[8] & 0x03) | (val << 2);
                wheel_report_data[9] = (wheel_report_data[9] & 0xF0) | (val >> 6);
                if (!manual_mode) send_now();
        }
        inline void sliderLeft(uint16_t val) {
                if (val > 1023) val = 1023;
                wheel_report_data[9] = (wheel_report_data[9] & 0x0F) | (val << 4);
                wheel_report_data[10] = (wheel_report_data[10] & 0xC0) | (val >> 4);
                if (!manual_mode) send_now();
        }
        inline void sliderRight(uint16_t val) {
                if (val > 1023) val = 1023;
                wheel_report_data[10] = (wheel_report_data[10] & 0x3F) | (val << 6);
                wheel_report_data[11] = (val >> 2);
                if (!manual_mode) send_now();
        }
        inline void slider(uint16_t val) {
                if (val > 1023) val = 1023;
                wheel_report_data[9] = (wheel_report_data[9] & 0x0F) | (val << 4);
                wheel_report_data[10] = (val >> 4) | (val << 6);
                wheel_report_data[11] = (val >> 2);
                if (!manual_mode) send_now();
        }
        inline void hat(int16_t dir) {
                ...
                wheel_report_data[4] = (wheel_report_data[4] & 0xF0) | val;
                if (!manual_mode) send_now();
        }
